...
func minTimeToReach(moveTime [][]int) int {
	...
	heap.Init(&pq)
	heap.Push(&pq, &Item{value: Point{x: 0, y: 0}, priority: 0})
	visited[0][0] = 0
	...
	for pq.Len() != 0 {
		...
		for i := 0; i < 4; i++ {
			...
			nextTime := max(moveTime[nextPoint.x][nextPoint.y], curTime) + 1

			if visited[nextPoint.x][nextPoint.y] == -1 || nextTime < visited[nextPoint.x][nextPoint.y] {
